...
export const getAllNotes = async (): Promise<Note[]> => {
  console.log('Fetching all notes from:', `${API_URL}/all`);  // Log the full endpoint
  try {
...
export const addNote = async (note: Pick<Note, 'text'>): Promise<Note> => {
...
export const updateNote = async (note: Note): Promise<string> => {
...
// function to send a get request to the backend to retrieve list of notes by their duedate
export const getNotesByDueDate = async (duedate: string): Promise<Note[]> => {
  console.log(`Fetching note with duedate ${duedate} from:`, `${API_URL}/get/duedate/${duedate}`);
  try {
    // Make an HTTP GET request to fetch notes by duedate
    const response = await fetch(`${API_URL}/get/duedate/${duedate}`);
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }
    // Parse the JSON data from the response
    const data = await response.json();
    // return fetched data as an array
    return data;
  } catch (error) {
    console.error('Error fetching note:', error);
    throw error;
